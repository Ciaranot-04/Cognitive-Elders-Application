...
import puzzles2 from '../puzzles/puzzles8.json';
...
export default function puz8() {
  ...
  const [puzzlelogs] = React.useState(() => {
  ...
  const [userspuzzles] = React.useState(() => {
    const i = Math.floor(Math.random() * puzzlelogs.length);
    return puzzlelogs[i];
  });
  ...
  const layout = userspuzzles.layout;
  const answer = userspuzzles.answer;
